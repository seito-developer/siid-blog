...
| thumbnailPreset | string[]（**microCMS では必須**） | サムネイルのジャンル選択（セレクトフィールド。選択肢はちょうど: `テック` / `キャリア` / `学習` / `Tips`）。選んだジャンルの画像（`web/public/thumbnails/{tech,career,learning,tips}.png`、1200x630）が eyecatch 未設定時に使われる。取得は `libs/article-thumbnail.ts` の `getArticleThumbnail()`。フィールドを持たない旧記事は記事 ID から決まる既定画像にフォールバック（旧選択肢 preset-01〜04 も後方互換で解決） |
...
- ~~**サムネイルのプリセット化**~~ — 対応済み（#13 + PR #33 フォローアップ）。microCMS 側の**必須作業**: blog API にセレクトフィールド `thumbnailPreset`（表示名例: サムネイルのジャンル、**必須・単数選択**、選択肢はちょうど `テック` / `キャリア` / `学習` / `Tips`）を追加する。選んだジャンルの画像がアイキャッチとして使われる（アップロード式 eyecatch があればそちらを優先）
