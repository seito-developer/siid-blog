...
| eyecatch | { url, width, height }（任意） | アイキャッチ画像（microcms-assets.io 配信）。未設定時は `libs/article-thumbnail.ts` の `getArticleThumbnail()` がプリセット画像（`web/public/thumbnails/preset-01〜04.png`、1200x630）にフォールバック |
| thumbnailPreset | string[]（任意） | プリセットサムネイルの選択（microCMS セレクトフィールド。選択肢: preset-01〜04）。eyecatch が無い場合に使用。未設定なら記事 ID から決まる既定プリセット |
...
- ~~**サムネイルのプリセット化**~~ — フロントエンド側は対応済み（#13。プリセット4種 + `getArticleThumbnail()` によるフォールバック。eyecatch 未設定でも壊れない）。microCMS 側の任意作業: blog API にセレクトフィールド `thumbnailPreset`（選択肢: preset-01 / preset-02 / preset-03 / preset-04、任意入力）を追加すると記事ごとに選べる。追加しなくても記事 ID から自動で決まる
